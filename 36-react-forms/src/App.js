...
class App extends Component {
  ...
  upvoteSelectedPainting = () => {
    this.setState({
      paintings: this.state.paintings.map(painting => {
        if (painting.id !== this.state.selectedPaintingId) return painting;
        painting.votes++
        return painting;
      })
    })
      ...
      <div className="App">
...

        {
          this.state.selectedPaintingId ?
            <PaintingDetails upvote={this.upvoteSelectedPainting} clearSelectedPainting={() => this.setState({ selectedPaintingId: undefined })} {...this.selectedPainting()} /> :
